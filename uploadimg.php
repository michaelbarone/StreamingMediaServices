...
$MAX_FILES_SIZE 	= '10M';
$ALLOW_EXTENSIONS 	= array("png","jpg","gif");
$UPLOAD_PATH 		= "upload/$userid/poster";
$OVERRIDE 			= false;
$ALLOW_DELETE		= true;
...
class RealAjaxUploader
{
	...
	public $allow_ext 		= array("mp4","m4v","mov");
	public $max_file_size 	= '2G';
	...
	function __construct($deny_ext=array(),$log)
	{
		$this->log=$log;
	...
	public function uploadFile()
	{
		if($this->checkFile())//this checks every chunk FIXME is right?
		{
			...
			$this->log->LogInfo("File uploaded could not check file");
		}
	}
	...
	private function finish()
	{
		...
		$this->log->LogInfo("File uploaded - $this->upload_path$this->file_name");
	...
	private function checkFile()
	{
		...
		if(!$this->checkSize())
		{
			$this->message(-1, 'File size exceeded maximum allowed: '.$this->max_file_size.'B');
		}
		return true;
	}
	...
	{
		if($msg!='Chunk uploaded') {
			$this->log->LogInfo("File upload Message - $this->file_name, $this->file_size has status:$status with message:$msg - more info:$extra_info");
		}
		$this->header();
...
$uploader = new RealAjaxUploader($DENY_EXT,$log);  //create uploader object
